...
def select_elements_starting_with_vowel(array)
  array.select{|string| string.start_with?('a','e','i','o','u') }
end
...
def remove_nils_and_false_from_array(array)
  array.reject{|item| item.nil? || !item}
end
...
def titleize_a_string(string)
  ...
  nocaps = ["and", "a", "the"]
  str.map { |word| nocaps.include?(word) ? word : word.capitalize }.join(" ")
end
...
def word_count_a_file(file_path)
  ...
  file.each_line{|line| count += line.scan(/[[:alpha:]]+/).count}
  return count  
end
...
def ninety_nine_bottles_of_beer
  ...
  num_start.downto(2) do |num| 
    puts "#{beer_calculator[num]} of beer on the wall, #{beer_calculator[num]} bottles of beer!"
     "Take one down, pass it around, #{beer_calculator[num - 1]} of beer on the wall"
  end 
